...
const NormalHard = ({ navigation }: RouterProps) => {
    ...
    const [numberOfTrash, setNumberOfTrash] = useState(1);
    const [numberOfHints, setNumberOfHints] = useState(1);
    const [incPowerUpCounter, setIncPowerUpCounter] = useState(0);
    ...
    function emptyContainer1() {
        ...
        else if(numberOfTrash <= 0) Alert.alert("Oops! You're out of trash power-ups. Try scoring more!");
        else if(numberOfTrash > 0 && wordHolder1.length > 0) {
            setWordHolder1([]);
            setNumberOfTrash(numberOfTrash - 1);
        }
    }
    ...
    function emptyContainer2() {
        ...
        else if(numberOfTrash > 0 && wordHolder2.length > 0) {
            setWordHolder2([]);
            setNumberOfTrash(numberOfTrash - 1);
        }
    }
    ...
    function emptyContainer3() {
        ...
        else if(numberOfTrash > 0 && wordHolder3.length > 0) {
            setWordHolder3([]);
            setNumberOfTrash(numberOfTrash - 1);
        }
    }
    ...
    function emptyContainer4() {
        ...
        else if(numberOfTrash > 0 && wordHolder4.length > 0) {
            setWordHolder4([]);
            setNumberOfTrash(numberOfTrash - 1);
        }
    }

    function updateNumberOfPowerUp() {
        setIncPowerUpCounter(incPowerUpCounter + 1);
        if(incPowerUpCounter == (powerUpEvery - 1)) {
            setNumberOfTrash(numberOfTrash + 1);
            setNumberOfHints(numberOfHints + 1);
            setIncPowerUpCounter(0);
        }
    }
    ...
    function showHints() {
        ...
        else Alert.alert("Oops! You're out of hints. Try scoring more!");
    }
    ...
    function exitGame() {
        Alert.alert("Exit Game?", "Are you sure you want to exit? \n" + "Current Score: " + score, [
            ...
            },
            {
                ...
                onPress: () => {
                    navigation.goBack();
                    updateHighScore(globalUserId, 0, score, 0);
                }
            }
        ])
    }
        ...
        <SQLiteProvider databaseName="words.db" assetSource={{ assetId: require('../../../assets/words.db') }}>
            ...
            <View style={styles.header_area}>
                ...
                <View style={styles.game_mode_container}>
                    <Text style={styles.game_mode_title}>Hard Mode</Text>
                </View>
                    ...
                    <View style={styles.trash_hints_container}>
                        <View style={styles.trash_container}>
                            ...
                            <View style={styles.number_of_trash_hints_container}> 
                                <Text style={styles.number_of_trash}>{numberOfTrash}</Text>
                            </View>
                        </View>
                        <View style={styles.hints_container}>
                            ...
                            <View style={styles.number_of_trash_hints_container}> 
                                <Text style={styles.number_of_trash}>{numberOfHints}</Text>
                            </View>
